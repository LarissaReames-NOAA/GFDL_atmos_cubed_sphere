...
! </table>
   ...
   real, dimension(:,:,:),allocatable,save :: dz_save
...
CONTAINS
  ...
  subroutine Riem_Solver3(ms, dt,   is,   ie,   js, je, km, ng,    &
                          ...
                          use_logp, last_call, fp_out,simpson,zh_save)
!--------------------------------------------
! !OUTPUT PARAMETERS
...
#endif
   real, intent(in), dimension(isd:ied,jsd:jed,km):: delp, pt, zh_save
...
! Local:
  real, dimension(is:ie,km):: dm, dz2, pm2, w2, gm2, cp2,dz2_old
  ...
  logical, save :: dzsaveisset
   ...
   peln1 = log(ptop)
     ptk = exp(akap*peln1)
   if (.not. allocated(dz_save)) then 
      allocate(dz_save(is:ie,js:je,km))
      dzsaveisset = .false.
   endif
!$OMP parallel do default(none) shared(is,ie,js,je,km,delp,ptop,peln1,pk3,ptk,akap,rgrav,zh,pt,dzsaveisset,dz_save,zh_save, &
...
!$OMP                          private(cp2, gm2, dm, dz2, pm2, pem, peg, pelng, pe2, peln2, w2,dz2_old)
#endif
   do 2000 j=js, je
...
#endif
             dm(i,k) = dm(i,k) * rgrav
            dz2_old(i,k) = zh_save(i,j,k+1) - zh_save(i,j,k)
      ...
      elseif ( a_imp > 0.999 ) then
           if (simpson) then
              call SIM_W_1D(dt, is, ie, km, rdgas, gama, gm2, cp2, akap, pe2,&
                            dm, pm2, pem, w2, dz2, pt(is:ie,j,1:km),ws(is,j),p_fac,j,dz2_old)
            else
