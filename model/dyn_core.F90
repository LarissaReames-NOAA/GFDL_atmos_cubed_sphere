...
public :: dyn_core, del2_cubed, init_ijk_mem
  ...
  real, allocatable, dimension(:,:,:) ::  ut, vt, crx, cry, xfx, yfx, divgd, &
                                          zh, du, dv, pkc, delpc, pk3, ptc,gz,zh_save
...
!-----------------------------------------------------------------------
 ...
 subroutine dyn_core(npx, npy, npz, ng, sphum, nq, bdt, n_map, n_split, zvir, cp, akap, cappa,  &
    ...
    integer, intent(in), optional:: time_total  !< total time (seconds) since start
...
!                    call init_ijk_mem(isd,ied, jsd,jed, npz, vt, 0.)
          ...
          if ( .not. hydrostatic ) then
               ...
               allocate( zh_save(isd:ied, jsd:jed, npz+1) )
         ...
         call update_dz_c(is, ie, js, je, npz, ng, dt2, dp_ref, zs, gridstruct%area, ut, vt, gz, ws3, &
           ...
           call Riem_Solver_C( ms, dt2,   is,  ie,   js,   je,   npz,   ng,   &
                               ...
                               q_con,  delpc, gz,  pkc, ws3, flagstruct%p_fac, &
                                flagstruct%a_imp,flagstruct%scale_z,simpson,time_total,it,init_step,zh)
...
!           call prt_maxmin('WS', ws, is, ie, js, je, 0, 1, 1., master)
        ...
        call Riem_Solver3(flagstruct%m_split, dt,  is,  ie,   js,   je, npz, ng,     &
                         ...
                         flagstruct%use_logp, remap_step,beta<-0.1,simpson,zh_save)
                                                         call timing_off('Riem_Solver')

                                       call timing_on('COMM_TOTAL')
        if ( flagstruct%fv_debug ) then
         if ( .not. flagstruct%hydrostatic )    then
            if(is_master()) print*, "AFTER RIEM_SOLVER"
            call prt_mxm('delz updated',  delz, is, ie, js, je, 0, npz, 1.,gridstruct%area_64, domain)
            call prt_maxmin('WS', ws, is, ie, js, je, 0, 1, 1.)
         endif
        endif
...
!----------------------------
    ...
    if ( hydrostatic ) then
       ...
       if ( flagstruct%fv_debug ) then
         if ( .not. flagstruct%hydrostatic )    then
            if(is_master()) print*, "AFTER NH_P_GRAD"
            call prt_mxm('U',  u, is, ie, js, je, 0, npz,1.,gridstruct%area_64, domain)
            call prt_mxm('V',  v, is, ie, js, je, 0, npz,1.,gridstruct%area_64, domain)
         endif
        endif
#ifdef ROT3
