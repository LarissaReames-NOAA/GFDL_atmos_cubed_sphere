...
! </table>
   ...
   real, dimension(:,:,:), allocatable, save :: dz_save_c
CONTAINS
...
! Enforce monotonicity of height to prevent blowup
  ...
  subroutine Riem_Solver_c(ms,   dt,  is,   ie,   js, je, km,   ng,  &
...
#endif
                           ptop, hs, w3,  pt, q_con, &
                           delp, gz,  pef,  ws, p_fac, a_imp,scale_m,simpson,t,it,init_step,zh)
   ...
   real, intent(in), dimension(is-ng:ie+ng,js-ng:je+ng,km):: pt, delp,zh
...
! Local:
  real, dimension(is-1:ie+1,km  ):: dm, w2, pm2, gm2, cp2, dz2_old,dz2
  ...
  logical, save :: dzsaveisset
   ...
   is1 = is - 1
   ie1 = ie + 1
   if(.not.allocated(dz_save_c)) then 
      allocate(dz_save_c(is-1:ie+1,js-1:je+1,km))
      dzsaveisset = .false.
   endif
!$OMP parallel do default(none) shared(js,je,is1,ie1,km,delp,pef,ptop,gz,rgrav,w3,pt,t,it,init_step,dz_save_c,dzsaveisset,zh, &
#ifdef MULTI_GASES
!$OMP                                  a_imp,dt,gama,akap,ws,p_fac,scale_m,ms,hs,q_con,cappa,kapad) &
!$OMP                          private(cp2,gm2, dm, w2, pm2, pe2, pem, peg, kapad2,dz2_old,dz2)
#else
!$OMP                                simpson,a_imp,dt,gama,akap,ws,p_fac,scale_m,ms,hs,q_con,cappa) &
!$OMP                          private(cp2,gm2, dm, dz2, w2, pm2, pe2, pem,peg,dz2_old)
#endif
   do 2000 j=js-1, je+1
...
! Excluding contribution from condensates:
      ...
      do k=1,km
         do i=is1, ie1
            dz2_old(i,k) = zh(i,j,k+1)-zh(i,j,k)
      ...
      elseif ( a_imp <= 0.5 ) then
        ...
        if ( .not. simpson) then
          ...
          call SIM_W_1D(dt, is1, ie1, km, rdgas, gama, gm2, cp2, akap, pe2,  &
                            dm, pm2, pem, w2, dz2, pt(is1:ie1,j,1:km),ws(is1,j), p_fac,j,dz2_old)
        endif
      endif
...
!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
subroutine SIM_W_1D(dt, is,ie,km, rgas, gama, gm2, cp2, kappa, pe, dm2, pm2, pem, w2, dz2, pt2, ws, p_fac,jslice,dz2_old)
   ...
   real,    intent(in ) ::  ws(is:ie)
   real,    intent(in ), dimension(is:ie,km+1):: pem
   real,    intent(inout), dimension(is:ie,km+1) ::  pe
   real,    intent(inout), dimension(is:ie,km) ::  dz2, w2
   real,    intent(in), dimension(is:ie,km) :: dz2_old
   integer, intent(in) :: jslice
! Local variables

   real, dimension(is:ie,km  ):: Ak, Bk, Rk, Ck, g_rat, bb, dd, aa, cc, dz1,w1,dz_save
   real, dimension(is:ie,km+1):: pp, gam, wE, dm2e, wES,zenew,zeold
...
! Compute non-hydrostatic pert pressure
   ...
   !    print*,  "jslice present and = 128"
    ...
    ENDIF
    !ENDIF
    IF (JSLICE==107 .AND. IS==43) then
      do k = 1,km
          write(01,*), k, pe(43,k), pm2(43,k)
          write(02,*), w2(43,k), dz2(43,k), dm2(43,k)
       enddo
       do k = 1,km+1
          write(04,*), pem(43,k)
       enddo
   ...
   do i = is,ie
      ...
      do k = km,1,-1
        zeold(i,k) = zeold(i,k+1)-dz2_old(i,k)
        zenew(i,k) = zenew(i,k+1)-dz2(i,k)
        if(jslice==128 .and. i==128 .and. k==1) print*, "zeold = ", zeold(i,k),dz2_old(i,k)
      enddo
   enddo
...
! Boundary conditions for wE = 0 at top
    ...
    do i = is,ie 
        ! Old Way
        !bet(i)    = bb(i,1)
        ! Could we pass in old dz to get some sort of dz/dt top BC?
        !wE(i,1)  = (zenew(i,1)-zeold(i,1))*rdt
        !wE(i,2)  = dd(i,1) / bet(i)
        !bb(i,km) = 2.
        !dd(i,km) = 3.*w1(i,km)

        ! New way with dw/dt=0 at top
        bb(i,1) = 2.0
        cc(i,1) = 1.0
        dd(i,1) = 3.0 * w1(i,1)
...
! Forward calculation of tri-diagonal system
    ...
    do i = is,ie 
      ...
      !wE(i,k+1) = (dd(i,k) - wE(i,k) ) / bet(i)
       ...
       bet(i) = aa(i,k) / bb(i,k-1)
       bb(i,k) = bb(i,k) - bet(i) * cc(i,k-1)
       dd(i,k) = dd(i,k) - bet(i) * dd(i,k-1)
    enddo     
    enddo
...
! Do the back substition, result is wE on zone edges.
    ...
    do i = is,ie 
      ...
      !wE(i,k) = wE(i,k) - gam(i,k+1)*wE(i,k+1)
       ...
       wE(i,k) = (dd(i,k) - cc(i,k) * wE(i,k+1)) / bb(i,k)
    enddo    
    enddo
...
!      wE(i,k)   = (Rk(i,k) - Ak(i,k) * wE(i,k-1) ) / bet(i)
       gam(i,k) = Ak(i,k)/Bk(i,k-1)
       Bk(i,k) = Bk(i,k) - gam(i,k) * Ck(i,k-1)
       Rk(i,k) = Rk(i,k) - gam(i,k) * Rk(i,k-1)
    enddo
    enddo
    
    do i = is,ie 
        wE(i,km) = Rk(i,km)/Bk(i,km)
        wE(i,km+1) = ws(i)
    enddo
...
! Back substitution for solution (wE = 0 at k=1)
    ...
    do i = is,ie   
      !wE(i,k) = wE(i,k) - gam(i,k+1)*wE(i,k+1)
      wE(i,k) = (Rk(i,k)-Ck(i,k)*wE(i,k+1))/Bk(i,k)
      !if (IEEE_IS_NAN(wE(i,k))) print*, jslice, i, k, wE(i,k)
    enddo       
    enddo
    do i = is,ie
       wE(i,1) =   wE(i,2)
...
! Need to generate new w at cell centers
    ...
    do i = is,ie 
        ...
        w2(i,k) = ( wE(i,k) + bb(i,k) * wE(i,k+1) * g_rat(i,k) * wE(i,k+2) ) *r3 - g_rat(i,k) * w2(i,k+1)
    enddo     
    enddo
...
! Do the back substition, result is pp on zone edges.
    ...
    ENDIF
    !ENDIF
    !print*, "j, is, ie, dz2 mean", jslice, is, ie, sum(dz2(is:ie,:))/size(dz2(is:ie,:))
